...
function Game(props) {
  ...
  const handleRate = async (act_id,rate) => {
    if (isLogin === false) {
      ...
      Axios.get(`https://team20-seffafkart.herokuapp.com/api/ratingadminact/act_id=${act_id}&user_id=${user[0].id}`)
      .then((response) => {
          console.log(response.data,"data")
          if(response.data.length !== 0) {
            if(response.data[0].rate_type === rate) {
              console.log("del time")
              Axios.put(`https://team20-seffafkart.herokuapp.com/api/deleterate/${response.data[0].id}`, {
                act_id: act_id,
                rate_type: rate
              }).then((response) => {
                  alert("deleted");
  ...
  useEffect(() => {
    ...
    Axios.get(`https://team20-seffafkart.herokuapp.com/api/comment_match/match_id=${match_id}`)
    .then(res => {
  ...
  const makeComment = () => {
    if (isLogin === false) {
      ...
      alert("You are an admin!");
        ...
        Axios.post("https://team20-seffafkart.herokuapp.com/api/makecomment", {
          content: comment,
